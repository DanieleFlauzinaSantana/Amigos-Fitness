...
import type { Student, DropoutPredictionResult, SurveyFeedbackForAI, SurveyAnswer, SurveyResponse } from '@/lib/types';
import { TrendingUp, TrendingDown, AlertTriangle, Sparkles, Lightbulb, ListChecks } from 'lucide-react';
import { Progress } from "@/components/ui/progress";
import { MOCK_SURVEY } from '@/lib/constants'; // Para mapear IDs de pergunta
...
export function DropoutPredictionSection({ student }: DropoutPredictionSectionProps) {
  ...
  const extractSurveyFeedback = (surveyResponse?: SurveyResponse): SurveyFeedbackForAI | undefined => {
    ...
    const getAnswerValue = (questionId: string): SurveyAnswer | undefined => 
      surveyResponse.answers.find(a => a.questionId === questionId);
    
    const q1Satisfaction = getAnswerValue('q1_satisfaction'); 
    if (q1Satisfaction && typeof q1Satisfaction.value === 'number') {
      feedback.overallSatisfaction = q1Satisfaction.value;
    }
    
    const q2Recommend = getAnswerValue('q2_recommend'); 
    if (q2Recommend && typeof q2Recommend.value === 'string' && (q2Recommend.value.toLowerCase() === 'sim' || q2Recommend.value.toLowerCase() === 'nao')) {
      feedback.wouldRecommend = q2Recommend.value.toLowerCase() as "sim" | "nao";
    }
    
    const q3Comments = getAnswerValue('q3_comments'); 
    if (q3Comments && typeof q3Comments.value === 'string') {
      feedback.comments = q3Comments.value;
    }
  ...
  const handlePredictDropout = async () => {
    ...
    const surveyFeedback = extractSurveyFeedback(student.latestSurveyResponse);
    ...
    const input: PredictDropoutInput = {
      ...
      profileData: {
        ...
        likesWinter: student.likesWinter,
        // Adicionar engagementLevel se tivermos esse dado no futuro
      },
      surveyFeedback: surveyFeedback,
    };

    console.log("Input para predictDropout IA:", JSON.stringify(input, null, 2));
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Analise o risco de desistência do aluno com base em seus dados, frequência e feedback.</CardDescription>
      </CardHeader>
