  ...
  const handleSurveySubmitted = (surveyResponse: SurveyResponse) => {
    if (studentId) {
      ...
      if (studentIndex !== -1) {
        ...
        console.log(`Resposta da pesquisa associada ao aluno ${students[studentIndex].name} e salva no localStorage.`);
      } else {
        console.warn(`Aluno com ID ${studentId} não encontrado no localStorage para salvar resposta da pesquisa.`);
      }
    } else {
        console.log("Pesquisa submetida anonimamente (sem studentId):", surveyResponse);
    }
  };
